    ...
    <div class="w-full grid xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 mt-4">
        ...
        <div
            ...
            <div class="p-4 grid xl:grid-cols-3 gap-4">
                ...
                <img src="{{ isset($student->profile_name) ? asset($student->profile_name) : 'https://www.gravatar.com/avatar/205e460b479e2e5b48aec07710c08d50?s=400' }}" 
                         alt="Student Photo" class="w-48 h-48 rounded-full object-cover border-4 border-primary/20">
                    <h2 class="text-xl font-semibold text-primary"> {{ $student->name }} {{ $student->student->lastname ?? '' }}</h2>
                    <span
                ...
                <div class="xl:col-span-2 flex flex-col gap-2">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <span class="font-medium">{{ $student->student->student_id ?? 'N/A' }}</span>
                        <span class="text-gray-600">Department:</span>
                        <span class="font-medium">{{ $student->student->department ?? 'N/A' }}</span>
                        <span class="text-gray-600">Principal Investigator:</span>
                        <span class="font-medium">{{ $student->student->pideatils->title ?? '' }} 
                                           {{ $student->student->piname->name ?? 'N/A' }} 
                                           {{ $student->student->pideatils->last_name ?? '' }}</span>
                        <span class="text-gray-600">Email:</span>
                        <span class="font-medium">{{ $student->email ?? 'N/A' }}</span>
                        <span class="text-gray-600">Phone:</span>
                        <span class="font-medium">{{ $student->student->mobile_number ?? 'N/A' }}</span>
                        <span class="text-gray-600">Address:</span>
                        <span class="font-medium">{{ $student->student->address ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
        </div>
